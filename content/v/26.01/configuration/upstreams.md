...
| `least_tokens_queued` | Route to target with lowest token queue (for [inference](../inference/)) |
...
Optimized for LLM/AI inference workloads. Selects the target with the lowest estimated queue time:
...
Features:
- Tracks queued tokens per target
- Maintains tokens-per-second EWMA (exponentially weighted moving average)
- Accounts for varying request sizes (token counts)

Best for:
- LLM inference backends (OpenAI, Anthropic, vLLM, etc.)
- Workloads with highly variable request processing times
- GPU clusters where throughput varies by model and batch size

See [Inference Configuration](../inference/) for complete setup including token rate limiting.
